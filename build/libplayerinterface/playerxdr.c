...
#include </home/benaissa/Téléchargements/player-3.0.2/build/libplayerinterface/playerxdr.h>
#include <string.h>
...
{   if(xdr_u_int(xdrs,&msg->key_count) != 1)
  ...
  if(xdr_char(xdrs,&msg->value) != 1)
    return(0);
  return(1);
}
int 
player_boolprop_req_pack(void* buf, size_t buflen, player_boolprop_req_t * msg, int op)
{
  XDR xdrs;
  int len;
  if(!buflen)
    return 0;
  xdrmem_create(&xdrs, buf, buflen, op);
  if(xdr_player_boolprop_req_t(&xdrs,msg) != 1)
    return(-1);
  if(op == PLAYERXDR_ENCODE)
    len = xdr_getpos(&xdrs);
  else
    len = sizeof(player_boolprop_req_t);
  xdr_destroy(&xdrs);
  return(len);
} 
unsigned int player_boolprop_req_t_copy(player_boolprop_req_t *dest, const player_boolprop_req_t *src)
{      
  ...
  memcpy(dest->key,src->key,sizeof(char)*src->key_count); 
  size += sizeof(char)*1;
  memcpy(&dest->value,&src->value,sizeof(char)*1); 
  return(size);
}
void player_boolprop_req_t_cleanup(const player_boolprop_req_t *msg)
{      
...
player_boolprop_req_t * player_boolprop_req_t_clone(const player_boolprop_req_t *msg)
{      
  player_boolprop_req_t * clone = malloc(sizeof(player_boolprop_req_t));
  if (clone)
    player_boolprop_req_t_copy(clone,msg);
  return clone;
}
void player_boolprop_req_t_free(player_boolprop_req_t *msg)
{      
  player_boolprop_req_t_cleanup(msg);
  free(msg);
}
unsigned int player_boolprop_req_t_sizeof(player_boolprop_req_t *msg)
{
...
{   if(xdr_float(xdrs,&msg->max_range) != 1)
  ...
  if(xdr_u_int(xdrs,&msg->angles_count) != 1)
    return(0);
  if(xdrs->x_op == XDR_DECODE)
  {
    if((msg->angles = malloc(msg->angles_count*sizeof(float))) == NULL)
      return(0);
  }
  {
    float* angles_p = msg->angles;
    if(xdr_array(xdrs, (char**)&angles_p, &msg->angles_count, msg->angles_count, sizeof(float), (xdrproc_t)xdr_float) != 1)
      return(0);
  }
  if(xdr_u_int(xdrs,&msg->intensity_count) != 1)
    return(0);
...
int 
player_laser_data_scanangle_pack(void* buf, size_t buflen, player_laser_data_scanangle_t * msg, int op)
{
  XDR xdrs;
  int len;
  if(!buflen)
    return 0;
  xdrmem_create(&xdrs, buf, buflen, op);
  if(xdr_player_laser_data_scanangle_t(&xdrs,msg) != 1)
    return(-1);
  if(op == PLAYERXDR_ENCODE)
    len = xdr_getpos(&xdrs);
  else
    len = sizeof(player_laser_data_scanangle_t);
  xdr_destroy(&xdrs);
  return(len);
} 
unsigned int player_laser_data_scanangle_t_copy(player_laser_data_scanangle_t *dest, const player_laser_data_scanangle_t *src)
{      
  ...
  memcpy(dest->ranges,src->ranges,sizeof(float)*src->ranges_count); 
  size += sizeof(uint32_t)*1;
  memcpy(&dest->angles_count,&src->angles_count,sizeof(uint32_t)*1); 
  if(src->angles != NULL && src->angles_count > 0)
  {
    if((dest->angles = malloc(src->angles_count*sizeof(float))) == NULL)
      return(0);
  }
  else
    dest->angles = NULL;
  size += sizeof(float)*src->angles_count;
  memcpy(dest->angles,src->angles,sizeof(float)*src->angles_count); 
  size += sizeof(uint32_t)*1;
  memcpy(&dest->intensity_count,&src->intensity_count,sizeof(uint32_t)*1); 
  if(src->intensity != NULL && src->intensity_count > 0)
  {
...
void player_laser_data_scanangle_t_cleanup(const player_laser_data_scanangle_t *msg)
{      
...
player_laser_data_scanangle_t * player_laser_data_scanangle_t_clone(const player_laser_data_scanangle_t *msg)
{      
  player_laser_data_scanangle_t * clone = malloc(sizeof(player_laser_data_scanangle_t));
  if (clone)
    player_laser_data_scanangle_t_copy(clone,msg);
  return clone;
}
void player_laser_data_scanangle_t_free(player_laser_data_scanangle_t *msg)
{      
  player_laser_data_scanangle_t_cleanup(msg);
  free(msg);
}
unsigned int player_laser_data_scanangle_t_sizeof(player_laser_data_scanangle_t *msg)
{
  ...
  size += sizeof(float)*msg->ranges_count; 
  size += sizeof(uint32_t)*1; 
  size += sizeof(float)*msg->angles_count; 
  size += sizeof(uint32_t)*1; 
  size += sizeof(uint8_t)*msg->intensity_count; 
  size += sizeof(uint32_t)*1; 
  return(size);
...
{      
  ...
  size += sizeof(uint32_t)*1;
  memcpy(&dest->index,&src->index,sizeof(uint32_t)*1); 
  size += sizeof(uint32_t)*1;
  memcpy(&dest->value_count,&src->value_count,sizeof(uint32_t)*1); 
  if(src->value != NULL && src->value_count > 0)
  {
...
{   if(xdr_vector(xdrs, (char*)&msg->pos, 8, sizeof(int32_t), (xdrproc_t)xdr_int) != 1)
    return(0);
  if(xdr_vector(xdrs, (char*)&msg->scale, 8, sizeof(int32_t), (xdrproc_t)xdr_int) != 1)
    return(0);
  if(xdr_u_int(xdrs,&msg->buttons) != 1)
    return(0);
  if(xdr_u_int(xdrs,&msg->axes_count) != 1)
    return(0);
  return(1);
}
int 
...
{   if(xdr_player_pose3d_t(xdrs,&msg->pose) != 1)
  ...
  if(xdr_player_pose3d_t(xdrs,&msg->vel) != 1)
    return(0);
  if(xdr_player_pose3d_t(xdrs,&msg->acc) != 1)
    return(0);
  return(1);
}
int 
player_imu_data_fullstate_pack(void* buf, size_t buflen, player_imu_data_fullstate_t * msg, int op)
{
  XDR xdrs;
  int len;
  if(!buflen)
    return 0;
  xdrmem_create(&xdrs, buf, buflen, op);
  if(xdr_player_imu_data_fullstate_t(&xdrs,msg) != 1)
    return(-1);
  if(op == PLAYERXDR_ENCODE)
    len = xdr_getpos(&xdrs);
  else
    len = sizeof(player_imu_data_fullstate_t);
  xdr_destroy(&xdrs);
  return(len);
} 
unsigned int player_imu_data_fullstate_t_copy(player_imu_data_fullstate_t *dest, const player_imu_data_fullstate_t *src)
{
  if (dest == NULL || src == NULL)
    return 0;
  memcpy(dest,src,sizeof(player_imu_data_fullstate_t));
  return sizeof(player_imu_data_fullstate_t);
} 
void player_imu_data_fullstate_t_cleanup(const player_imu_data_fullstate_t *msg)
{
} 
player_imu_data_fullstate_t * player_imu_data_fullstate_t_clone(const player_imu_data_fullstate_t *msg)
{      
  player_imu_data_fullstate_t * clone = malloc(sizeof(player_imu_data_fullstate_t));
  if (clone)
    player_imu_data_fullstate_t_copy(clone,msg);
  return clone;
}
void player_imu_data_fullstate_t_free(player_imu_data_fullstate_t *msg)
{      
  player_imu_data_fullstate_t_cleanup(msg);
  free(msg);
}
unsigned int player_imu_data_fullstate_t_sizeof(player_imu_data_fullstate_t *msg)
{
  return sizeof(player_imu_data_fullstate_t);
} 
...
{   if(xdr_double(xdrs,&msg->min_angle) != 1)
  ...
  if(xdr_double(xdrs,&msg->max_angle) != 1)
    return(0);
  if(xdr_double(xdrs,&msg->angular_res) != 1)
    return(0);
  if(xdr_double(xdrs,&msg->min_range) != 1)
    return(0);
...
{   if(xdr_player_pose3d_t(xdrs,&msg->pose) != 1)
  ...
  if(xdr_player_bbox3d_t(xdrs,&msg->size) != 1)
    return(0);
  if(xdr_u_int(xdrs,&msg->element_poses_count) != 1)
    return(0);
  if(xdrs->x_op == XDR_DECODE)
  {
    if((msg->element_poses = malloc(msg->element_poses_count*sizeof(player_pose3d_t))) == NULL)
      return(0);
  }
  {
    player_pose3d_t* element_poses_p = msg->element_poses;
    if(xdr_array(xdrs, (char**)&element_poses_p, &msg->element_poses_count, msg->element_poses_count, sizeof(player_pose3d_t), (xdrproc_t)xdr_player_pose3d_t) != 1)
      return(0);
  }
  if(xdr_u_int(xdrs,&msg->element_sizes_count) != 1)
    return(0);
  if(xdrs->x_op == XDR_DECODE)
  {
    if((msg->element_sizes = malloc(msg->element_sizes_count*sizeof(player_bbox3d_t))) == NULL)
      return(0);
  }
  {
    player_bbox3d_t* element_sizes_p = msg->element_sizes;
    if(xdr_array(xdrs, (char**)&element_sizes_p, &msg->element_sizes_count, msg->element_sizes_count, sizeof(player_bbox3d_t), (xdrproc_t)xdr_player_bbox3d_t) != 1)
      return(0);
  }
  return(1);
}
...
{      
  ...
  size += sizeof(uint32_t)*1;
  memcpy(&dest->element_poses_count,&src->element_poses_count,sizeof(uint32_t)*1); 
  if(src->element_poses != NULL && src->element_poses_count > 0)
  {
    if((dest->element_poses = malloc(src->element_poses_count*sizeof(player_pose3d_t))) == NULL)
      return(0);
  }
  else
    dest->element_poses = NULL;
  size += sizeof(player_pose3d_t)*src->element_poses_count;
  memcpy(dest->element_poses,src->element_poses,sizeof(player_pose3d_t)*src->element_poses_count); 
  size += sizeof(uint32_t)*1;
  memcpy(&dest->element_sizes_count,&src->element_sizes_count,sizeof(uint32_t)*1); 
  if(src->element_sizes != NULL && src->element_sizes_count > 0)
  {
    if((dest->element_sizes = malloc(src->element_sizes_count*sizeof(player_bbox3d_t))) == NULL)
      return(0);
  }
  else
    dest->element_sizes = NULL;
  size += sizeof(player_bbox3d_t)*src->element_sizes_count;
  memcpy(dest->element_sizes,src->element_sizes,sizeof(player_bbox3d_t)*src->element_sizes_count); 
  return(size);
}
...
{      
  ...
  if(msg == NULL)
    return;
  free(msg->element_poses); 
  free(msg->element_sizes); 
}
...
unsigned int player_ranger_geom_t_sizeof(player_ranger_geom_t *msg)
{
  ...
  size += sizeof(player_bbox3d_t)*1; 
  size += sizeof(uint32_t)*1; 
  size += sizeof(player_pose3d_t)*msg->element_poses_count; 
  size += sizeof(uint32_t)*1; 
  size += sizeof(player_bbox3d_t)*msg->element_sizes_count; 
  return(size);
}
...
{   if(xdr_player_ranger_data_range_t(xdrs,&msg->data) != 1)
    return(0);
  if(xdr_u_char(xdrs,&msg->have_geom) != 1)
    return(0);
  if(xdr_player_ranger_geom_t(xdrs,&msg->geom) != 1)
    return(0);
  if(xdr_u_char(xdrs,&msg->have_config) != 1)
    return(0);
  if(xdr_player_ranger_config_t(xdrs,&msg->config) != 1)
    return(0);
  return(1);
}
int 
player_ranger_data_rangestamped_pack(void* buf, size_t buflen, player_ranger_data_rangestamped_t * msg, int op)
{
  XDR xdrs;
  int len;
  if(!buflen)
    return 0;
  xdrmem_create(&xdrs, buf, buflen, op);
  if(xdr_player_ranger_data_rangestamped_t(&xdrs,msg) != 1)
    return(-1);
  if(op == PLAYERXDR_ENCODE)
    len = xdr_getpos(&xdrs);
  else
    len = sizeof(player_ranger_data_rangestamped_t);
  xdr_destroy(&xdrs);
  return(len);
} 
unsigned int player_ranger_data_rangestamped_t_copy(player_ranger_data_rangestamped_t *dest, const player_ranger_data_rangestamped_t *src)
{      
  ...
  size += sizeof(uint8_t)*1;
  memcpy(&dest->have_geom,&src->have_geom,sizeof(uint8_t)*1); 
  {size += player_ranger_geom_t_copy(&dest->geom, &src->geom);}
  size += sizeof(uint8_t)*1;
  memcpy(&dest->have_config,&src->have_config,sizeof(uint8_t)*1); 
  size += sizeof(player_ranger_config_t)*1;
  memcpy(&dest->config,&src->config,sizeof(player_ranger_config_t)*1); 
  return(size);
}
void player_ranger_data_rangestamped_t_cleanup(const player_ranger_data_rangestamped_t *msg)
{      
...
player_ranger_data_rangestamped_t * player_ranger_data_rangestamped_t_clone(const player_ranger_data_rangestamped_t *msg)
{      
  player_ranger_data_rangestamped_t * clone = malloc(sizeof(player_ranger_data_rangestamped_t));
  if (clone)
    player_ranger_data_rangestamped_t_copy(clone,msg);
  return clone;
}
void player_ranger_data_rangestamped_t_free(player_ranger_data_rangestamped_t *msg)
{      
  player_ranger_data_rangestamped_t_cleanup(msg);
  free(msg);
}
unsigned int player_ranger_data_rangestamped_t_sizeof(player_ranger_data_rangestamped_t *msg)
{
...
int 
player_ranger_data_intnsstamped_pack(void* buf, size_t buflen, player_ranger_data_intnsstamped_t * msg, int op)
{
  XDR xdrs;
  int len;
  if(!buflen)
    return 0;
  xdrmem_create(&xdrs, buf, buflen, op);
  if(xdr_player_ranger_data_intnsstamped_t(&xdrs,msg) != 1)
    return(-1);
  if(op == PLAYERXDR_ENCODE)
    len = xdr_getpos(&xdrs);
  else
    len = sizeof(player_ranger_data_intnsstamped_t);
  xdr_destroy(&xdrs);
  return(len);
} 
unsigned int player_ranger_data_intnsstamped_t_copy(player_ranger_data_intnsstamped_t *dest, const player_ranger_data_intnsstamped_t *src)
{      
...
void player_ranger_data_intnsstamped_t_cleanup(const player_ranger_data_intnsstamped_t *msg)
{      
...
player_ranger_data_intnsstamped_t * player_ranger_data_intnsstamped_t_clone(const player_ranger_data_intnsstamped_t *msg)
{      
  player_ranger_data_intnsstamped_t * clone = malloc(sizeof(player_ranger_data_intnsstamped_t));
  if (clone)
    player_ranger_data_intnsstamped_t_copy(clone,msg);
  return clone;
}
void player_ranger_data_intnsstamped_t_free(player_ranger_data_intnsstamped_t *msg)
{      
  player_ranger_data_intnsstamped_t_cleanup(msg);
  free(msg);
}
unsigned int player_ranger_data_intnsstamped_t_sizeof(player_ranger_data_intnsstamped_t *msg)
{
...
int 
player_pointcloud3d_stereo_element_pack(void* buf, size_t buflen, player_pointcloud3d_stereo_element_t * msg, int op)
{
  XDR xdrs;
  int len;
  if(!buflen)
    return 0;
  xdrmem_create(&xdrs, buf, buflen, op);
  if(xdr_player_pointcloud3d_stereo_element_t(&xdrs,msg) != 1)
    return(-1);
  if(op == PLAYERXDR_ENCODE)
    len = xdr_getpos(&xdrs);
  else
    len = sizeof(player_pointcloud3d_stereo_element_t);
  xdr_destroy(&xdrs);
  return(len);
} 
unsigned int player_pointcloud3d_stereo_element_t_copy(player_pointcloud3d_stereo_element_t *dest, const player_pointcloud3d_stereo_element_t *src)
{
  if (dest == NULL || src == NULL)
    return 0;
  memcpy(dest,src,sizeof(player_pointcloud3d_stereo_element_t));
  return sizeof(player_pointcloud3d_stereo_element_t);
} 
void player_pointcloud3d_stereo_element_t_cleanup(const player_pointcloud3d_stereo_element_t *msg)
{
} 
player_pointcloud3d_stereo_element_t * player_pointcloud3d_stereo_element_t_clone(const player_pointcloud3d_stereo_element_t *msg)
{      
  player_pointcloud3d_stereo_element_t * clone = malloc(sizeof(player_pointcloud3d_stereo_element_t));
  if (clone)
    player_pointcloud3d_stereo_element_t_copy(clone,msg);
  return clone;
}
void player_pointcloud3d_stereo_element_t_free(player_pointcloud3d_stereo_element_t *msg)
{      
  player_pointcloud3d_stereo_element_t_cleanup(msg);
  free(msg);
}
unsigned int player_pointcloud3d_stereo_element_t_sizeof(player_pointcloud3d_stereo_element_t *msg)
{
  return sizeof(player_pointcloud3d_stereo_element_t);
} 

int xdr_player_stereo_data_t (XDR* xdrs, player_stereo_data_t * msg)
{   if(xdr_player_camera_data_t(xdrs,&msg->left_channel) != 1)
    return(0);
  if(xdr_player_camera_data_t(xdrs,&msg->right_channel) != 1)
    return(0);
  if(xdr_player_camera_data_t(xdrs,&msg->disparity) != 1)
    return(0);
  if(xdr_u_int(xdrs,&msg->points_count) != 1)
    return(0);
  if(xdrs->x_op == XDR_DECODE)
  {
    if((msg->points = malloc(msg->points_count*sizeof(player_pointcloud3d_stereo_element_t))) == NULL)
      return(0);
  }
  {
    player_pointcloud3d_stereo_element_t* points_p = msg->points;
    if(xdr_array(xdrs, (char**)&points_p, &msg->points_count, msg->points_count, sizeof(player_pointcloud3d_stereo_element_t), (xdrproc_t)xdr_player_pointcloud3d_stereo_element_t) != 1)
      return(0);
  }
  if(xdr_u_int(xdrs,&msg->mode) != 1)
    return(0);
...
int 
player_stereo_data_pack(void* buf, size_t buflen, player_stereo_data_t * msg, int op)
{
  XDR xdrs;
  int len;
  if(!buflen)
    return 0;
  xdrmem_create(&xdrs, buf, buflen, op);
  if(xdr_player_stereo_data_t(&xdrs,msg) != 1)
    return(-1);
  if(op == PLAYERXDR_ENCODE)
    len = xdr_getpos(&xdrs);
  else
    len = sizeof(player_stereo_data_t);
  xdr_destroy(&xdrs);
  return(len);
} 
unsigned int player_stereo_data_t_copy(player_stereo_data_t *dest, const player_stereo_data_t *src)
{      
  
  unsigned int size = 0;
  if(src == NULL)
    return(0);
  {size += player_camera_data_t_copy(&dest->left_channel, &src->left_channel);}
  {size += player_camera_data_t_copy(&dest->right_channel, &src->right_channel);}
  {size += player_camera_data_t_copy(&dest->disparity, &src->disparity);}
  size += sizeof(uint32_t)*1;
  memcpy(&dest->points_count,&src->points_count,sizeof(uint32_t)*1); 
  if(src->points != NULL && src->points_count > 0)
  {
    if((dest->points = malloc(src->points_count*sizeof(player_pointcloud3d_stereo_element_t))) == NULL)
      return(0);
  }
  else
    dest->points = NULL;
  size += sizeof(player_pointcloud3d_stereo_element_t)*src->points_count;
  memcpy(dest->points,src->points,sizeof(player_pointcloud3d_stereo_element_t)*src->points_count); 
  size += sizeof(uint32_t)*1;
  memcpy(&dest->mode,&src->mode,sizeof(uint32_t)*1); 
  return(size);
}
void player_stereo_data_t_cleanup(const player_stereo_data_t *msg)
{      
  
  if(msg == NULL)
    return;
  player_camera_data_t_cleanup(&msg->left_channel); 
  player_camera_data_t_cleanup(&msg->right_channel); 
  player_camera_data_t_cleanup(&msg->disparity); 
  free(msg->points); 
}
player_stereo_data_t * player_stereo_data_t_clone(const player_stereo_data_t *msg)
{      
  player_stereo_data_t * clone = malloc(sizeof(player_stereo_data_t));
  if (clone)
    player_stereo_data_t_copy(clone,msg);
  return clone;
}
void player_stereo_data_t_free(player_stereo_data_t *msg)
{      
  player_stereo_data_t_cleanup(msg);
  free(msg);
}
unsigned int player_stereo_data_t_sizeof(player_stereo_data_t *msg)
{
  
  unsigned int size = 0;
  if(msg == NULL)
    return(0);
  {size += player_camera_data_t_sizeof(&msg->left_channel);}
  {size += player_camera_data_t_sizeof(&msg->right_channel);}
  {size += player_camera_data_t_sizeof(&msg->disparity);}
  size += sizeof(uint32_t)*1; 
  size += sizeof(player_pointcloud3d_stereo_element_t)*msg->points_count; 
  size += sizeof(uint32_t)*1; 
  return(size);
}
